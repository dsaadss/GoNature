...
import client.*;
import clientGUI.DmCancelledReport;
...
public class ChatClient extends AbstractClient
{
  ...
  /**
   * Processes messages received from the server.  Based on the command extracted from the message,
   * it performs  actions such as updating order details, handling user authentication status...
   
   * @param msg1 The message received from the server, expected to be a Message object or string.
   */
public void handleMessageFromServer(Object msg1) 
  {
	  ...
	  switch (result[0]) {
          ...
          case "OrderExistYarden":
        	  ...
        	  break;
          case"PriceAfter1":
        	  StaticClass.PriceforOrder=result[1];
        	  break;

		  case"getDataforCancelReportType1":
			  System.out.println("ChatClient>getDataforCancelReportType1");
			  DmCancelledReport.DataforCancelReportType1=(ArrayList<CancelReport>) payloadMessage.getPayload();
			  break;	  
	  }
  ...
  /**
   * Sends a message or command encapsulated within an Object from the client UI to the server. This method 
   * allows for more complex data types, beyond simple text messages, to be transmitted to the server for processing. 
   * It is particularly useful for sending serialized objects or commands that require structured data. The method 
   * ensures the connection to the server is open before sending and waits for a response to confirm the message 
   * was received and processed.
   *
   * @param message The Object containing the message or command to be sent. This could be a simple String, 
   *                a serialized object, or any other data structure that needs to be communicated to the server.
   */
